...
- **Canonical URLs.** Each page now declares a `rel="canonical"` to its `www.pennyhelm.com` address, so search engines consolidate ranking signals instead of splitting them between the apex and www hosts (both serve the site).
