...
- Unit test suite for `financial-service` and `recurring-service` — 106 tests covering net-worth math, monthly-income conversion, pay-date generation, bill expansion, financial-health score, Liquid Reserves (cash + taxable investments), Plaid bill-matching, merchant normalization, and recurring-transaction detection. Runs with `npm test` (Node built-in test runner, no new dependencies).
