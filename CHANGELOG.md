...
### Added
- **Shared mode: the app itself now scopes to your granted role.** When someone views finances shared with them, the sidebar collapses to just the shared overview plus their own Settings — a Companion no longer sees Bills, Calendar, Income, or any other tab their role doesn't include, and typing those URLs redirects to the overview. Accounts with a single share and no finances of their own land straight in the shared view on sign-in; everyone else gets a "Shared with you" section in the sidebar. "My finances" switches back at any time, and the viewing context is remembered across visits.
