...
### Added
- **First-run now leads with "Connect your bank."** Where bank sync is available (PennyHelm Cloud, or self-host with your own Plaid keys), the welcome screen's primary action connects a bank so accounts, balances, and debts import automatically — with "Add your first bill" opening the bill form as the manual path. The feature tour runs first and the chosen action fires as its final stop (immediately, if the tour is skipped). Sample data and start-fresh remain available; self-host without Plaid keys keeps the original welcome screen.
...
- **New "Repayment Strategy" planner under Debts.** An interactive avalanche payoff plan built around one-time lump sums: schedule lumps (add/edit/remove), set an ongoing monthly payment, and preview a refinance rate (HELOC / consolidation / 0% transfer). It shows debt-free date, interest from here, total out of pocket, a live payoff-horizon chart with a "cliff" at each lump, a per-lump breakdown of which debts each one clears, and the Avalanche-vs-Snowball comparison. You choose which debt **types** the plan covers (credit cards / auto loans / mortgages / any combination) on first open and can change it anytime, with per-debt include/exclude, "joint" and "refundable" flags, and editable plan notes. Reachable from the Debts tab bar or the "Open full planner" button on the Strategy Comparison card.
...
### Changed
- **No more native browser pop-ups anywhere.** Every remaining confirmation and alert — deleting a bill, debt, account, budget, savings goal, or expense; form-validation messages; bank-sync results — now uses the app's own styled confirm dialogs and toast notifications instead of the browser's default pop-ups.

_Also in this release (no behavior change): the bill↔transaction reconciliation logic and the expense-category list were consolidated into single shared modules, the dashboard's CSV/PDF export code was split into its own module, and the account/debt/bill linking service gained a full test suite._
...
- **The Expenses page has a Month / Quarter / Year toggle with back/forward navigation.** Arrows step one month (or quarter/year) at a time so older expenses can be found and edited; the totals, personal/business split, and top category follow the selected window; switching Month → Quarter keeps you on the period you were viewing; a Current button jumps back to today. Transfers stay visible in the list (badged) but excluded from the totals.
...
[Unreleased]: https://github.com/administrativetrick/pennyhelm/compare/v0.7.2...HEAD
[0.7.2]: https://github.com/administrativetrick/pennyhelm/compare/v0.7.1...v0.7.2
