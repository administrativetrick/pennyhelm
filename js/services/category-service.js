/**
 * Canonical expense/budget category DATA + pure lookups.
 *
 * This is the single source of truth for the category set, shared across the
 * web app, Cloud Functions, and the mobile app via `npm run sync:shared`.
 * It contains NO imports and NO DOM rendering so the sync transform can emit
 * a valid CommonJS copy (functions/) and a verbatim ESM copy (mobile/).
 *
 * The web-only HTML renderers (badge, <select> options, searchable picker)
 * live in js/expense-categories.js, which re-exports everything here.
 */
...
// Normalize a custom-category record into {key, name, color}. Accepts the
// stored shape ({id, key, name, color}) and defends the color to a hex literal
// (it lands in style="" attributes on web).
function normalizeCustom(c) {
    const key = c.key || String(c.name || '').toLowerCase().replace(/[^a-z0-9]+/g, '-');
    const color = /^#[0-9a-fA-F]{3,8}$/.test(c.color || '') ? c.color : '#94a3b8';
    return { key, name: c.name, color };
}

// Pull the custom-category array out of whatever the caller passed: a web
// `store` (has getCustomExpenseCategories()), a raw array (mobile passes
// `data.customExpenseCategories`), or nothing.
function customsOf(source) {
    if (!source) return [];
    if (Array.isArray(source)) return source;
    if (typeof source.getCustomExpenseCategories === 'function') return source.getCustomExpenseCategories();
    if (Array.isArray(source.customExpenseCategories)) return source.customExpenseCategories;
    return [];
}

/**
 * Normalize any user-supplied category string to a canonical key.
 *
 * Priority: exact key (case-insensitive) → exact label → slugified label →
 * pass through trimmed (so custom categories survive). Null/empty → null.
 *
 * @param {string} input
 * @param {object|Array} [source] — store, data, or custom-category array
 * @returns {string|null}
 */
export function normalizeCategoryKey(input, source) {
    if (input == null) return null;
    const raw = String(input).trim();
    if (raw === '') return null;

    const all = source ? getAllExpenseCategories(source) : EXPENSE_CATEGORIES;
...
/**
 * Built-in categories merged with the user's custom categories (under group
 * "Custom"). `source` may be a web store, a mobile data object, or a raw
 * custom-category array — see customsOf().
 */
export function getAllExpenseCategories(source) {
    const merged = { ...EXPENSE_CATEGORIES };
    for (const raw of customsOf(source)) {
        const c = normalizeCustom(raw);
        if (c.key && !merged[c.key]) {
            merged[c.key] = { label: c.name, group: 'Custom', color: c.color };
        }
    }
    return merged;
}

export function getCategoryLabel(key, source) {
    const all = source ? getAllExpenseCategories(source) : EXPENSE_CATEGORIES;
    return (all[key] || all['other']).label;
}

export function getCategoryColor(key, source) {
    const all = source ? getAllExpenseCategories(source) : EXPENSE_CATEGORIES;
    return (all[key] || all['other']).color;
}

/**
 * Flat, grouped list for building pickers: [{ value, label, group, color }]
 * in canonical order, built-ins first then custom. Handy for the mobile
 * screens, which render their own list UI.
 */
export function getCategoryOptionList(source) {
    const all = getAllExpenseCategories(source);
    return Object.entries(all).map(([key, cat]) => ({
        value: key, label: cat.label, group: cat.group, color: cat.color,
    }));
}
