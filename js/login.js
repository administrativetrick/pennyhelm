...
tabs.forEach(tab => {
    tab.addEventListener('click', () => {
        ...
        if (referralCodeGroup) {
            referralCodeGroup.style.display = isSignUp ? 'block' : 'none';
        }
...
googleBtn.addEventListener('click', async () => {
    ...
    const referralCode = isSignUp && referralCodeInput ? referralCodeInput.value.trim().toUpperCase() : '';
...
// Auto-fill referral code from URL param (?ref=REF-XXXXXXX)
const urlRef = new URLSearchParams(window.location.search).get('ref');
if (urlRef && referralCodeInput) {
    referralCodeInput.value = urlRef.toUpperCase();
    // Auto-switch to Sign Up tab so the referral field is visible
    if (!isSignUp) {
